...
object TrainingSchemas {
  ...
  /**
   * Defines the schema for the final BQML training tables in BigQuery. It contains only the
   * features and labels required for model training.
   */
  val BQMLTrainingDataSchema: StructType = StructType(
    Seq(
      // Features
