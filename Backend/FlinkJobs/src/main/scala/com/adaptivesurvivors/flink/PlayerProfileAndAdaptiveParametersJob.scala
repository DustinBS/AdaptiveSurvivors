...
/**
 * This Flink job is the central real-time engine for Adaptive Survivors. It consumes all gameplay
 * events and:
 *   1. Continuously generates adaptation parameters for enemies (Brute, Vexer) and sends them to
 *      the 'adaptive_params' topic.
 *   2. Emits full player feature vector when triggered by a 'seer_encounter_begin' or
 *      'elite_fight_completed event, and sends it to the 'bqml_features' topic, caches it in
 *      HDFS, and triggers the Python ML orchestrator.
 */
object PlayerProfileAndAdaptiveParametersJob {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val rawEventStream = env.fromSource(
      kafkaSource,
      WatermarkStrategy.noWatermarks(),
      "Gameplay Events Kafka Source")
    ...
    processedStream
      ...
      .name("Adaptive Params Kafka Sink")
...
class UnifiedProfileProcessor(
  ...
  override def open(parameters: Configuration): Unit = {
    ...
    hdfsFileSystem =
      org.apache.hadoop.fs.FileSystem.get(new java.net.URI(hdfsCachePath), hadoopConf)
    logger.info(s"HDFS FileSystem initialized for URI: ${hdfsFileSystem.getUri}")
  }

  override def processElement(
    event: GameplayEvent,
    ctx: KeyedProcessFunction[String, GameplayEvent, Unit]#Context,
    out: Collector[Unit]): Unit = {
    val profile =
      Option(profileState.value()).getOrElse(new PlayerProfile(event.player_id, event.run_id))

    val isFeatureTriggerEvent =
      event.event_type == "seer_encounter_begin" || event.event_type == "elite_fight_completed"
    if (isFeatureTriggerEvent) {
      val archetype =
        Try(event.payload.get("boss_archetype").toString.toLowerCase).getOrElse("none")
      if (archetype == "none") {
        logger.warn(
          s"Ignoring event '${event.event_type}' with 'none' archetype for run_id: ${ctx.getCurrentKey}")
    ...
    event.event_type match {
      case "player_movement_event" =>
        ...
        profile.lastPlayerVelocity =
          PredictionVector(dirPayload.get("dx").toFloat, dirPayload.get("dy").toFloat)
  ...
  override def onTimer(
    ...
    if (ts == profile.activeStaleTimer) {
      ...
      profile.stalenessOverrideUntil = ctx
        .timerService()
        .currentProcessingTime() + PlayerProfileAndAdaptiveParametersJob.stalenessGracePeriodMs
  ...
  private def updateBruteAdaptation(
    ...
    if (enemyMag > 0 && playerMag > 0) {
      val dotProduct =
        ((playerVx / playerMag) * (enemyVx / enemyMag)) + ((playerVy / playerMag) * (enemyVy / enemyMag))
      profile.recentKillDotProducts = (dotProduct :: profile.recentKillDotProducts).take(
        PlayerProfileAndAdaptiveParametersJob.maxDotProductHistory)
    }
  ...
  private def sendBruteFormUpdate(
    newForm: String,
    profile: PlayerProfile,
    ctx: KeyedProcessFunction[String, GameplayEvent, Unit]#Context): Unit = {
    ...
    val newTimer = ctx
      .timerService()
      .currentProcessingTime() + PlayerProfileAndAdaptiveParametersJob.stalenessCheckDurationMs
    ctx.timerService().registerProcessingTimeTimer(newTimer)
    profile.activeStaleTimer = newTimer
  }

  private def processFeatureVectorSnapshot(
    profile: PlayerProfile,
    eventPayload: java.util.Map[String, AnyRef],
    fightType: String,
    ctx: KeyedProcessFunction[String, GameplayEvent, Unit]#Context): Unit = {
    logger.info(
      s"Feature vector snapshot triggered by '$fightType' for run_id: ${ctx.getCurrentKey}. Processing...")

    val encounterId = Try(eventPayload.get("encounter_id").toString)
      .getOrElse(java.util.UUID.randomUUID().toString)
    val avgHp =
      if (profile.hpHistory.isEmpty) 1.0 else profile.hpHistory.sum / profile.hpHistory.size
